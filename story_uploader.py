...
transformers_for_expected_fan_fiction_metadata = {
    "Title": {
...
def required_metadata_was_collected(metadata_dictionary):
    for metadata_field in metadata_dictionary:
        if remaining_data_to_collect[metadata_field]["is_required"]:
            return False
    return True
...
story_file_name = sys.argv[1]

if not silence_script_progression_logs:
    print(">>> STARTED PROCESSING FILE: " + story_file_name)
    print("COLLECTING STORY METADATA...")

json_metadata_file_name = story_file_name.rstrip(".txt") + ".json"

with open(story_file_name, "r") as file_to_transform, open(json_metadata_file_name, "w") as new_file:
    new_file.write('{')
    remaining_data_to_collect = transformers_for_expected_fan_fiction_metadata.copy()
        ...
        if raw_line == "\n":
            ...
            if "Title" in remaining_data_to_collect:
                ...
                if (transformed_field in remaining_data_to_collect):
                    ...
                    new_file.write(f', "{transformed_field}": {transformed_value}')
...
if not silence_script_progression_logs:
    print("SUCCESSFULLY COLLECTED STORY METADATA!")
...
with open(story_file_name, "rb") as story_file:
    ...
    if s3_upload_response and "VersionId" in s3_upload_response and s3_upload_response["VersionId"]:
        s3_object_version_id = s3_upload_response["VersionId"]
        if not silence_script_progression_logs:
            print("S3 UPLOAD OF STORY FILE SUCCESSFUL!")
    else:
        print("ERROR: Failed processing as there was an issue with uploading the story file to S3. Double-check S3 parameters provided to this script as well as the S3 bucket configurations on AWS. Also verify whether your terminal is configured with the AWS CLI.")
        quit()

def rollback_s3_upload_from_failed_elastic_search_upload(reason):
    print("ERROR: " + reason)
...
try:
    ...
    rollback_s3_upload_from_failed_elastic_search_upload("There was an issue initializing the Elastic Search Client. Please verify that valid information has been provided for the elastic_search parameters in the upload_parameters.json")
...
try:
    ...
    rollback_s3_upload_from_failed_elastic_search_upload("Failed processing as there was an issue with uploading the file metadata to AWS ElasticSearch. Double-check elastic_search parameters provided to this script as well as the Elastic Search configurations on AWS.")

if successful_elastic_search_upload:
    if not silence_script_progression_logs:
        print("ELASTIC SEARCH UPLOAD OF STORY METADATA SUCCESSFUL!")
        print(">>> SUCCESSFULLY PROCESSED FILE: " + story_file_name)
else:
    rollback_s3_upload_from_failed_elastic_search_upload("Failed processing as there was an issue with uploading the file metadata to AWS ElasticSearch. Double-check elastic_search parameters provided to this script as well as the Elastic Search configurations on AWS.")
